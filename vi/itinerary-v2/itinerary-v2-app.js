// itinerary-v2-app.js — UI logic for v2 mobile-first planner (Bản tiếng Việt)
    ...
    card.innerHTML = `
      <div class="itin-cover" data-ph="${it.cover}">
        ...
        <div class="itin-cover-foot">
          <div class="days-big">${it.days}<small>ngày</small></div>
...
function buildLibrary(){
  ...
  shown.forEach(s=>{
    ...
    card.draggable = true;
    const iconicBadge = s.iconic ? `<span class="lib-card-iconic">★ Nổi bật</span>` : '';
    ...
    card.innerHTML = `
      <div class="lib-card-img" data-ph="${s.img}">
        ...
        <button class="lib-card-add" aria-label="Thêm">${added?'✓':'+'}</button>
      </div>
      <div class="lib-card-body">
        ...
        <div class="lib-card-meta">
          ...
          ${s.overnight && s.cat!=='stay' ? `<span class="lib-card-stay">🌙 Nghỉ qua đêm</span>` : ''}
        </div>
      </div>`;
...
function openAddSheet(spotId){
  ...
  document.getElementById('sheet-head-title').textContent = isAdded ? 'Đã trong lịch trình' : 'Thêm vào chuyến đi';
  document.getElementById('sheet-head-sub').textContent  = isAdded ? 'Bạn có thể di chuyển ngày khác hoặc xóa.' : 'Chọn ngày để thêm địa điểm này.';
  ...
  body.innerHTML = `
    <div class="sheet-detail">
      ...
      <div class="sheet-detail-text">
        <h4>${sp.name}</h4>
        <div class="km">${sp.km}km từ homestay</div>
        <div class="cat">${capCat(sp.cat)}${sp.iconic?' · <strong style="color:#E8A000">★ Nổi bật</strong>':sp.tag?` · ${sp.tag}`:''}</div>
        ${sp.maps?`<a class="ml" href="${sp.maps}" target="_blank" rel="noopener">📍 Xem Google Maps</a>`:''}
      </div>
    </div>
    <div class="sheet-section-lab">${isAdded ? 'Đang ở ngày' : 'Thêm vào'}</div>
    <div id="sheet-day-list"></div>
    <button class="sheet-add-new" onclick="addToNewDay()">+ Thêm vào ngày mới</button>
    ${isAdded?`<button class="sheet-remove" onclick="removeFromAll()">Xóa khỏi lịch trình</button>`:''}
  `;
  ...
  days.forEach((d,i)=>{
    ...
    btn.innerHTML = `
      ...
      <div class="info">
        <div class="t">Ngày ${i+1}</div>
        <div class="s">${d.length} địa điểm${isIn?' · đã ở đây':''}</div>
      </div>
      <span class="ck">✓</span>`;
...
function addToNewDay(){
  ...
  days.push([id]);
  closeSheet();
  toast(`Đã thêm vào ngày mới (Ngày ${days.length})`);
  render();
}
...
function addDay(){
  days.push([]);
  toast(`Đã thêm Ngày ${days.length}`);
  render();
}
function removeDay(di, ev){
  ev?.stopPropagation();
  if(days.length<=1){ days[0]=[]; render(); return; }
  if(!confirm('Xóa ngày này và tất cả địa điểm đã chọn?')) return;
  days.splice(di,1);
  render();
}
  ...
  days.forEach((day, di)=>{
    ...
    block.innerHTML = `
      <div class="day-hd" style="background:${color}" onclick="toggleDayCollapse(${di})">
        <div class="dl">
          <span class="dn">Ngày ${di+1}</span>
          <span class="ds">${day.length} điểm dừng · ~${Math.round(dayKm)}km</span>
        </div>
        <div class="ac">
          ${editMode?`<button class="day-rm" onclick="removeDay(${di}, event)" title="Xóa ngày này">✕</button>`:''}
    ...
    if(day.length===0){
      body.innerHTML = '<div class="empty-day">Chọn các địa điểm phía dưới để thêm vào ngày này</div>';
    } else {
      day.forEach((id,si)=>{
        ...
        if(si === 0){
          if(prevId === 'home'){
            legLabel = `${legKm}km từ homestay`;
          } else {
            const prevSp = spotById(prevId);
            legLabel = `${legKm}km từ điểm nghỉ ${prevSp ? prevSp.name : 'qua đêm'}`;
          }
        } else {
          legLabel = `${legKm}km từ điểm ${si}`;
        }
        ...
        const overnightBadge = isOvernight
          ? '<span class="overnight-badge">Nghỉ qua đêm</span>'
          : '';
        ...
        row.innerHTML = `
          ...
          <div class="stop-acts">
            <button onclick="moveStop(${di},${si},-1,event)" ${si===0?'disabled':''} aria-label="Lên">↑</button>
            <button onclick="moveStop(${di},${si},1,event)" ${si===day.length-1?'disabled':''} aria-label="Xuống">↓</button>
            <button onclick="removeStop(${di},${si},event)" aria-label="Xóa">✕</button>
          </div>`;
      ...
      if(stayInfo){
        ...
        stays.innerHTML = `
          <div class="day-stays-hd">Nghỉ qua đêm gần khu vực ${stayInfo.spot.name}</div>
...
function renderMap(){
  ...
  days.forEach((day,di)=>{
    ...
    if(day.length>0){
      ...
      pill.innerHTML = `<div class="sw" style="background:${color}"></div>Ngày ${di+1} · ${day.length} điểm`;
...
function updateWA(){
  const km = planTotalKm(days);
  let msg = `Chào Sen! Đây là lịch trình Cao Bằng mình tự thiết kế (${days.length} ngày, tổng cộng ~${Math.round(km)}km):\n\n`;
  days.forEach((day,di)=>{
    msg += `Ngày ${di+1}:\n`;
    if(day.length===0){ msg += `  (nghỉ ngơi thư thả)\n`; }
    day.forEach((id,si)=>{
      const sp = spotById(id);
  ...
  msg += `Sen báo giá trọn gói xe máy, hướng dẫn viên và góp ý thêm giúp mình nhé. Cảm ơn Sen!`;
...
async function exportItineraryImage(){
  ...
  const pillTxt = "SEN'S HOMESTAY  ·  CAO BẰNG, VIỆT NAM";
  ctx.save();
  ...
  const line1 = titleParts.length > 1 ? titleParts[0].trim() : 'Lịch trình Cao Bằng';
  const line2 = titleParts.length > 1 ? titleParts[1].trim() : 'Tự thiết kế';
  ...
  const statItems = [
    {v: String(days.length), l: 'NGÀY'},
    {v: String(totalStops), l: 'ĐIỂM DỪNG'},
    {v: `~${totalKm}km`, l: 'TỔNG QUÃNG ĐƯỜNG'},
  ];
  const sW=220, sH=60, sGap=18;
  ...
  days.forEach((day,di)=>{
    ...
    t(`Ngày ${di+1}`, textX, dhMid, `italic 700 38px "Cormorant Garamond", Georgia, serif`, color);
    const dayKm = Math.round(chainedDayKm(di));
    t(`${stops} điểm dừng  ·  ~${dayKm}km`, cX+cW-28, dhMid, `400 20px "DM Sans", system-ui, sans-serif`, C_TM, 'right');
    ...
    if(stops===0){
      t('Chưa thêm địa điểm nào — chọn địa điểm phía dưới nha', W/2, cy+DAY_HEAD_H+STOP_H/2,
      ...
      day.forEach((id,si)=>{
        ...
        if(isReturnHome){
          t('↩ Quay về Sen\'s Homestay', nameX, midY-12,
            `600 26px "DM Sans", system-ui, sans-serif`, C_GREEN, 'left', nameMaxW);
          t(`~${legKm}km quay về homestay`, nameX, midY+15,
          ...
          if(si === 0){
            if(prevId === 'home'){
              ...
              const prevSp = spotById(prevId);
              const prevName = prevSp ? prevSp.name : 'điểm nghỉ qua đêm';
              legLabel = `${legKm}km từ ${prevName}`;
            }
          } else {
            legLabel = `${legKm}km từ điểm dừng trước`;
          }
        ...
        if(isOvernight){
          ...
          t('Nghỉ qua đêm ☾', bx + bW/2, by + bH/2 + 1,
    ...
    if(stayInfo){
      ...
      t(`Nghỉ qua đêm ☾ · khu vực ${stayInfo.spot.name}`, cX+ACCENT_W+28, headerY,
  ...
  t('Gửi lịch trình này cho Sen để nhận báo giá trọn gói', W/2, fy+36, `400 22px "DM Sans", system-ui, sans-serif`, C_TM, 'center');
  t('wa.me/84822946888', W/2, fy+70, `700 32px "DM Sans", system-ui, sans-serif`, C_ORANGE, 'center');
  t("Sen's Homestay · Cao Bằng, Việt Nam", W/2, fy+100, `400 18px "DM Sans", system-ui, sans-serif`, '#bbb', 'center');

  const link = document.createElement('a');
  link.download = `caobang-itinerary-${days.length}ngay.png`;
  link.href = canvas.toDataURL('image/png');
  link.click();
  toast('Đã lưu ảnh lịch trình!');
}
